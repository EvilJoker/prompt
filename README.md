...
| `维护` `optimize` `性能` | `09-optimization.md` | 持续化 | 系统优化维护 |
| `架构` `architecture` | `10-architecture-sync.md` | 步骤化 | 架构同步 |
| **`开源项目检查` `opensource check`** | **`10-opensource.md`** | **4阶段式** | **开源项目完整性检查与创建** |
...
用户: 需求分析用户登录系统
系统: 📋 执行信息
      - 应用规则：02-requirements.md (需求分析流程)
...
用户: 开源项目检查
系统: 📋 执行信息
      - 应用规则：10-opensource.md (开源项目完整性检查)
      - 执行模式：4阶段式(扫描→分析→建议→创建)
      - 质量评分：100分制评分体系
...
用户: dev 基于设计文档实现登录功能
系统: 📋 执行信息
      - 应用规则：04-develop.md (代码开发流程)
      - 加载依赖：02-common-standards.md, 03-design.md
      - 执行模式：5步骤化执行
```
...
└── rules/                     # 具体规则文件
    ...
    └── 10-opensource.md      # 开源项目完整性检查与创建：4阶段式
```
...
规则依赖图:
  ...
  10-opensource.md:           # 开源项目检查 (10号规则)
    depends: [quality-templates.md, 02-common-standards.md]
    description: 开源项目完整性检查与文档创建助手
